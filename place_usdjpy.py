"""Place USDJPY Asia-NY v3 orders. Run 22:05 UTC Mon-Thu (after ref window closes).
Ref: prior calendar day 19-22 UTC high/low. Sell MIT @ H+1.25*ATR10 (SL 1*ATR, TP 0.75R),
Buy MIT @ L-1.25*ATR. Flat next morning 06:00 UTC bar close via end_usdjpy.py."""
import argparse
import os

import pandas as pd

import lab_live as L

BUF, MULT, TP = 1.25, 1.0, 0.75
BOOK = "USDJPY"


def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--dry-run", action="store_true")
    args = ap.parse_args()
    L.load_env()

    now = pd.Timestamp.now(tz="UTC")
    if now.weekday() > 3:
        print(f"weekday={now.weekday()} — Mon-Thu evening sessions only, skipping")
        return
    if L.load_session(BOOK):
        print("session already placed tonight — idempotent skip")
        return

    df = L.load_hourly("USDJPY=X", period="12d")
    hi = lo = None
    for back in range(1, 8):   # walk back to the previous TRADING day (Mon evening -> Fri)
        try:
            day = (now - pd.Timedelta(days=back)).strftime("%Y-%m-%d")
            hi, lo = L.window_levels(df, 19, 22, day=day)
            break
        except SystemExit:
            continue
    if hi is None:
        raise SystemExit("no reference window found in last 7 days")
    atr = L.atr_last(df, 10)
    sell_trig = hi + BUF * atr
    buy_trig = lo - BUF * atr
    stop_d = MULT * atr
    tp_d = TP * stop_d
    account = float(L.os.environ.get("ACCOUNT_SIZE", "10000"))
    risk_usd = account * float(L.os.environ.get("RISK_PCT", "1.0")) / 100.0
    price_mid = (hi + lo) / 2
    units = round(risk_usd * price_mid / stop_d, 2)   # JPY quote convention (per place_orders.py)

    state = {"book": BOOK, "day": now.strftime("%Y-%m-%d"), "atr": atr,
             "ref_hi": hi, "ref_lo": lo,
             "orders": [
                 {"transaction": "sellShort", "trigger": sell_trig,
                  "sl": sell_trig + stop_d, "tp": sell_trig - tp_d, "units": units},
                 {"transaction": "buy", "trigger": buy_trig,
                  "sl": buy_trig - stop_d, "tp": buy_trig + tp_d, "units": units}],
             "status": "placed"}
    print(f"ATR10 {atr:.3f} | prev NY-late H {hi:.3f} L {lo:.3f}")
    print(f"SELL MIT {sell_trig:.3f} | BUY MIT {buy_trig:.3f} | SL {stop_d:.3f} TP {tp_d:.3f}")

    if args.dry_run:
        print("DRY RUN — nothing placed")
        return

    from etoro_client import EtoroClient
    cli = EtoroClient()
    inst = cli.resolve(os.environ.get("ETORO_JPY_SYMBOL", "USDJPY"))
    pf = cli.portfolio()
    pending = [o for o in (pf.get("clientPortfolio", {}).get("orders") or [])
               if o.get("instrumentID") == inst["instrumentId"]]
    if pending:
        print("orders already resting on eToro — skip")
        return

    for o in state["orders"]:
        r = cli.place_mit(inst, o["transaction"], o["trigger"], o["sl"], o["tp"], o["units"],
                          leverage=10)
        o["order_id"] = r.get("orderId")
        o["reference_id"] = r.get("referenceId")
        L.append_fill_log({"ts": now, "book": BOOK, "event": "place",
                           "side": o["transaction"], "price": o["trigger"], "sl": o["sl"],
                           "tp": o["tp"], "units": o["units"], "order_id": o.get("order_id")})
    L.save_session(state, BOOK)
    L.notify(f"USDJPY Asia-NY armed — {now.strftime('%a %d %b')}",
             f"SELL MIT {sell_trig:.3f} · BUY MIT {buy_trig:.3f}\n"
             f"SL 1×ATR ({stop_d:.3f}) · TP 0.75R · flat 06:00 UTC",
             fields=[{"name": "Risk", "value": f"{risk_usd:.0f} USD ({units} units)",
                      "inline": True},
                     {"name": "ATR10", "value": f"{atr:.3f}", "inline": True}])


if __name__ == "__main__":
    main()
